...
class ChemyxFusion4kXPump(object):
    ...
    def setDiameter(self, x, diameter):
        command = str(x) + ' ' + 'set diameter ' + str(diameter)
        response = self.sendCommand(command)
        return response

    def setRate(self, x, rate):
        self.x = x
        if float(rate) <= 0:
            self.pausePump(x)
        ...
        # else:
        command = str(x) + ' ' + 'set rate ' + str(rate)
        response = self.sendCommand(command)
        return response
